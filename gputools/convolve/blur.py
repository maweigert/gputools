...
def blur(data, width=4., normalize=True, res_g=None):
    ...
    if isinstance(data, OCLArray):
        return _blur_buf(data, width, res_g, normalize=normalize)
    elif isinstance(data, np.ndarray):
        return _blur_np(data, width, normalize=normalize)

    else:
        raise TypeError("unknown type (%s)" % (type(data)))


def _blur_buf(d_g, width=(4., 4.), res_g=None, normalize=True):
    Ns = tuple((int(3 * s + 1) // 2) * 2 + 1 for s in width)
    sigmas = tuple(s / 2.35 for s in width)

    hs = tuple(
        np.exp(-.5 / s ** 2 * np.linspace(-N / 2., N / 2., N) ** 2) for s, N in zip(reversed(sigmas), reversed(Ns)))
...
def _blur_np(data, width, normalize=True):
    d_g = OCLArray.from_array(data.astype(np.float32, copy=False))

    return _blur_buf(d_g, width, normalize=normalize).get()
